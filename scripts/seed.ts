 ...
 *
 * 가상 학교 하나를 통째로 만든다. is_demo 플래그가 붙어 있고 school_id 가
 * 다르므로, RLS 때문에 실제 학교 데이터와는 어떤 경로로도 섞이지 않는다.
 *
 * 다시 실행하면 기존 데모 학교를 지우고 새로 만든다.
 */
...
async function main() {
  ...
  console.log(`  교직원 ${staffIds.length}명`)
  ...
  console.log(`  시정표 ${periodRows.length}줄 (과정마다 시각이 다름)`)
  ...
  console.log(`  학급 ${classes?.length ?? 0}개`)
  ...
  console.log(`  특별실 ${rooms?.length ?? 0}개`)
  ...
  console.log(`  시간표 ${slotRows.length}칸`)
  ...
  console.log(`  특별실 예약 ${reservationRows.length}건 (승인 대기·선택과목 포함)`)
  ...
  console.log(`  결과 ${absencePlan.length}건 → 보강 대상 ${assignmentCount}개`)
  ...
  console.log(`  학사일정 ${eventRows.length}건`)
  ...
  console.log(`  학생 ${studentRows.length}명 (가명처리 — 실명 없음)`)
  ...
  for (const account of DEMO_ACCOUNTS) {
    console.log(`  ${account.email.padEnd(24)} ${DEMO_PASSWORD}   ${account.note}`)
  }
