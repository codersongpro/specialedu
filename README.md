...
| 시간표 보기 (교사별·학급별) | 됨 |
| 교직원 초대·권한, 접속 기록 | 됨 |
| 예산 관리 (품의·집행·부서별 집행률) | 됨 |
| PBS 기록·통계 | 됨 |
| 수업 도구함 (쉬운글+그림 안내문, 유튜브 자료 찾기) | 됨 |
...
실제 학교가 이미 등록돼 있어도 만들 수 있습니다 — `school_id` 가 다른
별도 학교로 들어가 RLS 로 완전히 격리되기 때문입니다. 데모를 다 만든
뒤에는 `SEED_SECRET` 환경변수를 지우는 편이 안전합니다(교직원이 늘 볼
필요는 없는 경로라서).
