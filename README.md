# 한아름 — 특수학교 교직원 업무 지원 앱

"한 아름"(두 팔 벌려 가득 안는다)에서 딴 이름이다. 특별실 예약, 결보강 배정,
학사일정을 한 곳에서 처리한다.
...
이미 배포한 사이트라면 로컬에 아무것도 깔지 않고 만들 수 있습니다.
`SEED_SECRET` 환경변수를 넣고 배포한 뒤:

```bash
curl -X POST https://<주소>/api/demo/seed \
  -H "Authorization: Bearer <SEED_SECRET>"
```

`SEED_SECRET` 이 없으면 이 경로는 아예 동작하지 않습니다(기본 잠김).
실제 학교가 이미 등록돼 있으면 거부합니다. 데모를 다 만든 뒤에는
환경변수를 지우는 편이 안전합니다.

가상 학교 「한빛특수학교」가 만들어집니다. 실존 인물 정보는 없습니다.
데모 데이터가 들어가면 **로그인 화면에 역할별 둘러보기 버튼**이 나타납니다.
비밀번호를 입력하지 않고 눌러서 바로 들어갈 수 있습니다.
